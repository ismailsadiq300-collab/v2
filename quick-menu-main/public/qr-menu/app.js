...
// Supabase Configuration for waiter calls
const SUPABASE_URL = 'https://llkvfataaxatjeahpyat.supabase.co';
const SUPABASE_ANON_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Imxsa3ZmYXRhYXhhdGplYWhweWF0Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NjY0NzE4OTQsImV4cCI6MjA4MjA0Nzg5NH0.XP4FM7-CQ71THMBaGLTIsH3Q91V4KkuIvyovBpxcIno';
...
let isFirstWaiterCallLoad = true;
let waiterCallsChannel = null;
...
// Flash browser title for new orders/waiter calls
let titleFlashInterval = null;
function flashTitle(count, type = 'Order') {
  const originalTitle = document.title;
  const alertTitle = `🔔 ${count} New ${type}${count > 1 ? 's' : ''}!`;
...
// Waiter Calls Functions (Supabase)
async function loadWaiterCalls() {
  const section = document.getElementById('waiter-calls-section');
  const container = document.getElementById('waiter-calls-list');
  const countBadge = document.getElementById('waiter-calls-count');
  
  // Unsubscribe from previous channel if exists
  if (waiterCallsChannel) {
    supabaseClient.removeChannel(waiterCallsChannel);
  }
  ...
  waiterCallsChannel = supabaseClient
    ...
    .on(
      ...
      async (payload) => {
        ...
        // Reload all active calls
        const { data: updatedCalls } = await supabaseClient
          .from('waiter_calls')
          .select('*')
          .in('status', ['pending', 'acknowledged'])
          .order('created_at', { ascending: false });
        ...
        if (payload.eventType === 'INSERT' && payload.new.status === 'pending') {
          console.log('🔔 New waiter call from Table', payload.new.table_number);
          playNotificationSound();
          flashTitle(1, 'Waiter Call');
        }
        
        renderWaiterCalls(updatedCalls || []);
      }
    )
    .subscribe();
}
...
function renderWaiterCalls(calls) {
  ...
  container.innerHTML = calls.map(call => `
    <div class="waiter-call-card" data-id="${call.id}">
      <div class="waiter-call-info">
        <span class="waiter-call-table">🔔 Table #${call.table_number}</span>
        <span class="waiter-call-time">${formatWaiterCallTime(call.created_at)}</span>
      </div>
      <span class="waiter-call-status ${call.status}">${call.status}</span>
      <div class="waiter-call-actions">
        ${call.status === 'pending' ? `
          <button class="waiter-call-btn acknowledge" onclick="updateWaiterCallStatus('${call.id}', 'acknowledged')">
            On my way
          </button>
        ` : ''}
        ${call.status === 'acknowledged' ? `
          <button class="waiter-call-btn complete" onclick="updateWaiterCallStatus('${call.id}', 'completed')">
            Done
          </button>
        ` : ''}
      </div>
...
async function updateWaiterCallStatus(callId, newStatus) {
  try {
...
function formatWaiterCallTime(timestamp) {
  if (!timestamp) return '';
